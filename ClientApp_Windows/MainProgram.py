...
AppUI.set_appearance_mode(theme)  # 可為 "light", "dark", 或 "system"
AppUI.set_default_color_theme("blue")  # 主題色

# --- 建立主視窗 ---
root = AppUI.CTk()
root.title(_("Smart Drink Workshop Control Panel"))
root.attributes('-fullscreen', True)
root.bind("<Escape>", lambda e: root.iconify()) # 可選：允許按 ESC 離開全螢幕
...
def change_theme(choice):
    AppUI.set_appearance_mode(choice)

# --- 介面元件設計 ---
title_label = AppUI.CTkLabel(root, text=_("Smart Drink Workshop Control Panel"), font=("Arial", 28))
title_label.pack(pady=20)

# --- 主控制區塊 ---
frame = AppUI.CTkFrame(root)
frame.pack(pady=10, padx=20, fill="both", expand=True)

function_label = AppUI.CTkLabel(frame, text=_("Please select a function"), font=("Arial", 24))
function_label.pack(pady=10)

motor_btn = AppUI.CTkButton(frame, text=_("Start Motor"), font=("Arial", 20), command=lambda: run_command("motor"))
motor_btn.pack(pady=10)

camera_btn = AppUI.CTkButton(frame, text=_("Take Photo"), font=("Arial", 20), command=lambda: run_command("camera"))
camera_btn.pack(pady=10)

shutdown_btn = AppUI.CTkButton(frame, text=_("Shutdown"), font=("Arial", 20), fg_color="darkred", hover_color="red",
                              command=lambda: run_command("shutdown_windows"))
shutdown_btn.pack(pady=10)

# --- 語言選單（下拉選單）---
lang_label = AppUI.CTkLabel(root, text=_("Language"), font=("Arial", 16))
lang_label.pack(pady=(20, 5))

lang_menu = AppUI.CTkOptionMenu(root, values=["zh_TW", "en"], command=change_language)
lang_menu.set(lang)  # 預設語言
lang_menu.pack()

# --- 主題選單（下拉選單）---
theme_label = AppUI.CTkLabel(root, text=_("Theme"), font=("Arial", 16))
theme_label.pack(pady=(20, 5))

theme_menu = AppUI.CTkOptionMenu(root, values=["light", "dark"], command=change_theme)
